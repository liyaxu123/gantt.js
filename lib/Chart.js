...
class Chart {
  ...
  createGanttChart(wrapper) {
    ...
    $ganttChart.addEventListener('click', (event) => {
      ...
      if (target.className === addPrefixCls('task-item-content')) {
  ...
  renderTaskItem(data) {
    ...
    $taskItem.appendChild($resizeHolderLeft);
    ...
    $taskItem.appendChild($resizeHolderRight);
  ...
  taskItemResizeHolderLeft() {
    ...
    handleDrag($resizeHolder, {
      onDragBefore: () => {
        $resizeHolder.style.visibility = 'visible';
      },
      onDraging: (steps) => {
        const $taskItem = $resizeHolder.parentElement;
      ...
      onDragEnd: () => {
        $resizeHolder.removeAttribute('style');
      },
    }, this.step);
    return $resizeHolder;
  }
