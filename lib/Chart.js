...
class Chart {
  ...
  taskItemProgressHandle(progress) {
    ...
    handleDrag($taskItemProgressHandle, {
      ...
      onDraging: (deltaX, ev) => {
        if (ev.target.classList.contains(addPrefixCls('task-item-progress-wrapper'))) {
          const taskItemWidth = ev.target.offsetWidth;
          $taskItemProgressText = ev.target.firstChild;

          let progressPercent = ev.offsetX / taskItemWidth;
          ...
          const res = `${Math.floor(progressPercent * 100)}%`;
          $taskItemProgressText.style.display = 'block';
          $taskItemProgressText.innerHTML = res;
          ev.target.style.background = `linear-gradient(to right, transparent ${res}, rgba(0, 0, 0, 0.1) ${res} 100%)`;
          $taskItemProgressHandle.style.left = res;
